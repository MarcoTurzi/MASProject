...
public class AdaptiveOM extends OpponentModel{
	...
	// Bayesian Model that compute preference hypothesis and then generate a probability distribution based on time
	...
	private void computeHypothesis(int n){
		...
		//every hypothesis is composed by the possible combination of weight and issue value
	...
	// method to initialize the distribution so that at the beginning ever hypothesis has the same probability
	...
	// methods to generate combinations of values
	...
	//time function used to compute the conditional probability of every hypothesis
	public double utilityTime(double time) {
		
		return 1- 0.05*time;
		
	}
	//method that compute the conditional probability based on this formula
	// (1/sqrt(2pi)* o)*e^((hpUtil - timeUtil)^2)/2*o*o
	...
	protected void updateModel(Bid bid, double time) {
		...
		// Update the opponentUtilitySpace using the value contained in the most likely hypothesis
